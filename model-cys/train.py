...
def run(model_name: str, num_speakers=None):
    ...
    if model_name == "mfccnn":
        ...
        file_path = f"/data/cuilab/AAI/data_cache/mfcc_64_{num_speakers}.pkl"
    ...
    if args.noise:
        file_path = f"/data/cuilab/AAI/data_cache/mfcc_224_{num_speakers}_{args.SNR}.pkl"
    else:
        file_path = f"/data/cuilab/AAI/data_cache/mfcc_224_{num_speakers}.pkl"
    ...
    if os.path.exists(file_path):
        ...
        for i in range(1, 1 + num_speakers):
            data_all.append(
                load_audio_files(
                    f"/data/cuilab/AAI/LibriSpeech-SI/train/spk{str(i).zfill(3)}/")
            )
        ...
        for speaker in tqdm(data_all):
            ...
            for spk_aud in speaker:
                ...
                noise_lst = random.sample(noise_all, num_noise+5)
                ...
                x = np.sqrt(sum_s/(sum_n * pow(10, args.SNR/10)))
        ...
        for i, data in enumerate(tqdm(data_addnoise)):
            ...
            mfccs_val = create_mfccs(
                x_val_i, mfcc_transformer, WINDOW_SIZE, OFFSET)
            mfccs_test = create_mfccs(
                x_test_i, mfcc_transformer, WINDOW_SIZE, OFFSET)
    ...
    if log_file:
        with open(log_file, "a") as f:
            print("Test Loss = %.5f" % test_loss,
                  "Test acc = %.5f " % test_acc, file=f)
    save_path = f"./saved_model/{model._get_name()}_{num_speakers}_state_dict.pt"
    if args.noise:
        save_path = f"./saved_model/{model._get_name()}_{num_speakers}_state_dict_{args.SNR}.pt"
    torch.save(
        model.state_dict(),
        save_path,
    )
...
def predict(model_name, file_list, save_path=None):
    ...
    if args.noise:
        param_path = f"./saved_model/{model._get_name()}_250_state_dict_{args.SNR}.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", default="resnet", type=str, required=False)
    parser.add_argument("-n", type=int, required=False, default=250)
    parser.add_argument("-g", default=0, type=int, required=False)
    parser.add_argument("-p", action="store_true", required=False)
    parser.add_argument("-f", type=str, required=False)
    # parser.add_argument("-noise",action="store_false",default=True, required=False)
    parser.add_argument("-noise", action="store_true",
                        default=False, required=False)
    parser.add_argument("-SNR", default=15, type=int, required=False)
    parser.add_argument("-save", action="store_false",
                        default=True, required=False)

    args = parser.parse_args()
    if args.noise:
        print("use noise audio!")
    GPU_ID = args.g
    ...
    if args.p:
        if args.f:
            ...
            test_path = "/data/cuilab/AAI/LibriSpeech-SI/test/"
            save_path = f"{model_name}_prediction.txt"
            if args.noise:
                test_path = "/data/cuilab/AAI/LibriSpeech-SI/test-noisy/"
                save_path = f"{model_name}_prediction_noisy.txt"
            # test_path = "/data/cuilab/AAI/LibriSpeech-SI/test-noisy/"
            file_list = [
                os.path.join(test_path, file) for file in sorted(os.listdir(test_path))
            ]
            predict(model_name, file_list, save_path)
    else:
